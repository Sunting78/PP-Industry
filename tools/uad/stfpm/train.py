...
def argsparser():
    ...
    parser.add_argument("--weight_decay", type=float, default=None, help='weight_decay')
...
def main():
    ...
    image_list = sorted(glob(os.path.join(args.data_path, args.category, 'train', 'good', '*.png')))
    ...
    if args.do_eval:
        train(teacher, student, train_loader, val_loader, args,  test_pos_loader, test_neg_loader)
    else:
        train(teacher, student, train_loader, val_loader, args)


def train(teacher, student, train_loader, val_loader, args, test_pos_loader=None, test_neg_loader=None):
    min_err = 10000
    teacher.eval()
    student.train()
    ...
    for epoch in range(args.epochs):
        ...
        if args.do_eval:
            val(args, student, teacher, test_pos_loader, test_neg_loader, epoch=epoch, eval_pro=args.compute_pro)
