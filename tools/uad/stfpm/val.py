...
from ppindustry.uad.datasets.mvtec import MVTecDatasetSTFPM, load_gt
from ppindustry.uad.utils.utils import eval_metric
...
def argsparser():
    ...
    parser.add_argument("--model_path", type=str, default=None, help="student checkpoint")
...
def main():
    ...
    test_neg_image_list = sorted(glob(os.path.join(args.data_path, args.category, 'test', 'good', '*.png')))
    test_pos_image_list = set(glob(os.path.join(args.data_path, args.category, 'test', '*', '*.png'))) - set(
        test_neg_image_list)
    test_pos_image_list = sorted(list(test_pos_image_list))
    test_neg_dataset = MVTecDatasetSTFPM(test_neg_image_list, transform=transform)
    test_pos_dataset = MVTecDatasetSTFPM(test_pos_image_list, transform=transform)
    test_neg_loader = DataLoader(test_neg_dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=args.num_workers)
    test_pos_loader = DataLoader(test_pos_dataset, batch_size=1, shuffle=False, drop_last=False, num_workers=args.num_workers)

    saved_dict = paddle.load(args.model_path)
    print('load ' + args.model_path)
    student.load_dict(saved_dict)

    val(args, student, teacher, test_pos_loader, test_neg_loader, epoch=None, eval_pro=args.compute_pro)

def cal_error(args, teacher, student, loader):
    teacher.eval()
    student.eval()
    loss_map = np.zeros((len(loader.dataset), args.resize[0]//4, args.resize[1]//4))
    i = 0
    for batch_data in loader:
        ...
        for j in range(len(t_feat)):
            ...
            sm = F.interpolate(sm, size=(args.resize[0]//4, args.resize[1]//4), mode='bilinear', align_corners=False)
...
def val(args, student, teacher, test_pos_loader, test_neg_loader, epoch=None, eval_pro=False):
    ...
    if eval_pro: ## very slow
        ...
        auc_pixel = eval_metric(gt_pixel.flatten(), scores.flatten(), metric='roc')
        auc_image_max = eval_metric(gt_image, scores.max(-1).max(-1), metric='roc')
        if epoch != None:
