...
from ppindustry.uad.utils.utils import str2bool
...
def argsparser():
    ...
    parser.add_argument("--k", type=int, default=None, help="used feature channels")
    parser.add_argument("--method", type=str, default=None,
                        choices=['sample', 'h_sample', 'ortho', 'svd_ortho', 'gaussian', 'coreset'],
...
def main():
    ...
    # build model
    model = get_model(args.method)(arch=args.backbone, pretrained=True, k=args.k, method=args.method)
    ...
    for i, class_name in enumerate(class_names):
        ...
        # build datasets
        train_dataset = mvtec.MVTecDataset(args.data_path, class_name=class_name, is_train=True, resize=args.resize,
...
def train(args, model, train_dataloader, class_name):
    ...
    if args.save_model:
        print("Saving model...")
        save_name = os.path.join(args.save_path + f"/{args.method}_{args.backbone}_{args.k}", '{}.pdparams'.format(class_name))
