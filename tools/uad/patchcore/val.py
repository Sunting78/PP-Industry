...
from ppindustry.uad.utils.utils import *
...
def argsparser():
    ...
    parser.add_argument('--eval_threthold_step', type=int, default=500,
...
def main():
    ...
    csv_name = os.path.join(args.save_path + f"/{args.method}_{args.backbone}_{args.k}", '{}_seed{}.csv'.format(args.category, args.seed))
    for i, class_name in enumerate(class_names):
        print("Eval model {}/{} for {}".format(i + 1, len(class_names), class_name))

        # build model
        model_path = args.model_path or args.save_path + f"/{args.method}_{args.arch}_{args.k}" + '/{}.pdparams'.format(class_name)
        ...
        # build datasets
        test_dataset = mvtec.MVTecDataset(args.data_path, class_name=class_name, is_train=False, resize=args.resize,
                                          cropsize=args.crop_size)
        test_dataloader = DataLoader(test_dataset, batch_size=args.test_batch_size, num_workers=args.num_workers)
        result.append([class_name, *val(args, model, test_dataloader, class_name)])
...
def val(args, model, test_dataloader, class_name):
    ...
    per_pixel_auroc = compute_roc_score(gt_mask.flatten(), score_map.flatten(), args.eval_threthold_step,
    ...
    total_PRO = compute_pro_score(gt_mask, score_map, args.eval_threthold_step,
                                  args.non_partial_AUC) if args.eval_PRO else None

    print([class_name, img_auroc, per_pixel_auroc, total_PRO])
    if args.save_pic:
        save_dir = os.path.join(args.save_path + f"/{args.method}_{args.backbone}_{args.k}", class_name)
