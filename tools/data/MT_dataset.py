...
DEFECTS_LABELS = {'Free': 0, # free is backgroud 
                  'Blowhole': 1,
                  'Break': 2,
                  'Crack': 3,
                  'Fray': 4,
...
def parse_args():
    ...
    parser.add_argument('--dataset_path', type=str, help="path to folder of Magnetic-Tile dataset")
    parser.add_argument('--output_path', type=str, default="./dataset/MT_dataset/", help="where to store output")
    ...
    for name, label_id in DEFECTS_LABELS.items():
        ...
        for img_name in os.listdir(folder_path):
            ...
            if exp_prefix in TRAIN_PREFIX:
                ...
                line = osp.join(train_img_path, img_name) + " " + osp.join(train_anno_path, anno_name) + "\n"
                f_train.write(line)
            elif exp_prefix in VAL_PREFIX:
                shutil.copyfile(osp.join(folder_path, img_name), osp.join(val_img_path, img_name))
                cv2.imwrite(osp.join(val_anno_path, anno_name), anno_img)
                line = osp.join(val_img_path, img_name) + " " + osp.join(val_anno_path, anno_name) + "\n"
                f_val.write(line)
            else:
