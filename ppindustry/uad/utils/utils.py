...
def compute_pro_score(y_true: np.ndarray, amaps: np.ndarray, steps=500, non_partial_AUC=False) -> float:
    ...
    return compute_non_partial_auc(rescale(fprs), rescale(pros)) if non_partial_AUC else auc(rescale(fprs), rescale(
        pros))  # compute_non_partial_auc(fprs, rescale(pros), 0.3)
